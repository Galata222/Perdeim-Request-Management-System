...
<head>
    ...
    <title>Vice President Approval</title>
...
<?php
  ...
  $data=$conn->query("select * from college");
  $data1=$conn->query("select * from college");
  $id=$name=$date=$offices=$program=$salary=$rate=$purpose=$departure=$destination=$means=$totaldays=$birr=$fuel=$incidental=$total=$head_name=$dept_id=$approved_birr=$college_id=$dean_name="";
  $sql2="SELECT * FROM college where col_notif=1";
  ...
  $new_data=$conn->query("select * from college where col_notif=1");
  $count=mysqli_num_rows($new_data);
$sql="SELECT * FROM request_info where college_id='$college_id'";
...
if($result1->num_rows>0){
  ...
  header("Location:vice_president.php?notification= Employee id is: $emp_id <br> The name of employee: $name <br> submitted date: $date <br>  College/School/Department/Offices: $offices <br> Salary $salary <br> Perdiem rate per day birr: $rate <br>  Purpose of the trip: $purpose <br> Departure: $departure <br> Destination $destination <br> Means of travel: $means<br> Total no. of days: $totaldays <br> Advance payment <br>perdiemm payment birr: $birr <br>Fuel: $fuel<br>Incidental: $incidental<br>Total pay birr: $total<br> Deparment Id: $dept_id<br> Approved by Dept head: $head_name <br> Total Approved birr:$approved_birr <br> College ID: $college_id <br> College Dean Name:$dean_name");
        ...
        <div class="container style">
          ...
          <ul class="dropdown-menu">
            ...
            if($value['col_notif']=='1'){
              $id=$value['dean_name'];
            ?>
            <li style="width: 200px" class="alert-danger"><a href="?notf=<?php echo $value['dean_name'];?>"><?php echo "NEW Request the from " .$value['dean_name']; ?></a></li>
          
          <?php 
            } else{
            ?>
            <li style="width: 200px"><?php echo "NEW Request from" .$value['dean_name']; ?></li>
          <?php
        }}
          ?>
            </ul>
...
<div class="container">
  <form class="form"  action="president_db.php" method="POST">

  <p class="form-label">
      Enter the employee ID <input type="number" name="id" class="form-control">
    </p>
    <p class="form-label">
      Enter the Vice President office ID <input type="number" name="vice_id" class="form-control">
    </p>
    <p class="form-label">
      Enter Vice President Name<input type="text" name="name" class="form-control">
    </p>
  ...
  <form action="dept_disapprove_db.php" method="Post" class="comment">
    ...
    <p class="form-label">
     Vice President ID<input type="number" name="vp_id" class="form-control">
    </p>
    <p class="form-label">
      Name of Vice President<input type="text" name="vp_name" class="form-control">
    </p>
